...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function sendMessage() {
        ...
        if (hasImage) {
            imageData = clientApi.getImageData(currentImageId);
        ...
        try {
            ...
            const response = await clientApi.generateResponse(chatHistory, options);

            removeTypingIndicator();
            addBotMessage(response);
            
            // Clean up image data
            if (hasImage && currentImageId) {
                clientApi.removeImageData(currentImageId);
            }
        } catch (error) {
            removeTypingIndicator();
